...
public final class RestClient {
	...
	public <T> T call(String methodName, String restPath, Class<T> expectedResponse, Object payload,
		...
		Response result = webTarget.request().headers(headers).method(methodName,
		...
		if (result != null && result.hasEntity() && isStatusCodeOK(result)) {
	...
	private boolean isStatusCodeOK(Response response) {
		if (response.getStatus() == Status.OK.getStatusCode()
			...
			LOG.error("{} - {} on ressource {}", errorResponse.getException(),
					errorResponse.getMessage() + " :::: " + errorResponse.getRessource());
		} else {
	...
	private WebTarget createWebTarget(String restPath, Map<String, String> queryParams) {
		...
		try {
			...
			if (queryParams != null && !queryParams.isEmpty()) {
				webTarget = verificarparametros(webTarget, queryParams);
			}
	...
	private WebTarget verificarparametros(WebTarget webTarget, Map<String, String> queryParams) {
	...
	private Client createSLLClient(ClientConfig clientConfig) {
		...
		try {
			...
			client = ClientBuilder.newBuilder().sslContext(sc).hostnameVerifier(new HostnameVerifier() {
			...
			LOG.error("Error en la generacion certificado: ", e);
			return client;
